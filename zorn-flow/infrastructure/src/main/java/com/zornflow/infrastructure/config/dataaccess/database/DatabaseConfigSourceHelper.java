...
public class DatabaseConfigSourceHelper {
   ...
   * 保存规则链配置
   */
  public void saveRuleChainConfig(DSLContext dsl, RuleChainConfig config, String ruleChainsTable, String ruleChainRulesTable) {
    // 1. 保存或更新规则链基本信息
    dsl.insertInto(table(ruleChainsTable))
    ...
    // 2. 删除旧的规则关联
    dsl.deleteFrom(table(ruleChainRulesTable))
   ...
   * 保存流程链配置
   */
  public void saveProcessChainConfig(DSLContext dsl, ProcessChainConfig config, String processChainsTable, String processChainNodesTable, String gatewayConditionsTable) {
    // 1. 保存或更新流程链基本信息
    dsl.insertInto(table(processChainsTable))
      .set(field("id"), config.id())
      .set(field("name"), config.name())
      .set(field("version"), config.version())
      .set(field("description"), config.description())
    ...
    // 2. 删除旧的节点关联和网关条件
    dsl.deleteFrom(table(gatewayConditionsTable))
      .where(field("process_chain_id").eq(config.id()))
      .execute();
    dsl.deleteFrom(table(processChainNodesTable))
      .where(field("process_chain_id").eq(config.id()))
      .execute();
    ...
    if (config.nodes() != null && !config.nodes().isEmpty()) {
      for (int i = 0; i < config.nodes().size(); i++) {
        ...
        saveProcessChainNode(dsl, config.id(), node, i + 1, processChainNodesTable);

        // 保存网关条件（如果是Gateway节点）
        if (node.type() == ProcessNodeConfig.NodeType.GATEWAY &&
          node.conditions() != null && !node.conditions().isEmpty()) {
          saveGatewayConditions(dsl, config.id(), node.id(), node.conditions(), gatewayConditionsTable);
        }
      }
    }
  }
   ...
   * 保存全局规则
   */
  public void saveGlobalRule(DSLContext dsl, GlobalRuleConfig config, String globalRulesTable) {
    dsl.insertInto(table(globalRulesTable))
      .set(field("id"), config.id())
      .set(field("name"), config.name())
   ...
   * 保存全局流程节点
   */
  public void saveGlobalProcessNode(DSLContext dsl, ProcessNodeConfig config, String globalNodesTable) {
    dsl.insertInto(table(globalNodesTable))
      .set(field("id"), config.id())
      .set(field("name"), config.name())
  ...
  private void saveRuleChainRule(DSLContext dsl, String ruleChainId, RuleConfig rule, int order, String ruleChainRulesTable) {
    // 这里简化处理，实际中需要判断是否为全局引用
    boolean isGlobalReference = false; // 根据实际逻辑判断

    dsl.insertInto(table(ruleChainRulesTable))
  ...
  private void saveProcessChainNode(DSLContext dsl, String processChainId, ProcessNodeConfig node, int order, String processChainNodesTable) {
    // 这里简化处理，实际中需要判断是否为全局引用
    boolean isGlobalReference = false; // 根据实际逻辑判断

    dsl.insertInto(table(processChainNodesTable))
  ...
  private void saveGatewayConditions(DSLContext dsl, String processChainId, String nodeId, List<GatewayConditionConfig> conditions, String gatewayConditionsTable) {
    for (int i = 0; i < conditions.size(); i++) {
      GatewayConditionConfig condition = conditions.get(i);
      dsl.insertInto(table(gatewayConditionsTable))
        .set(field("process_chain_id"), processChainId)
